...
Compared with v2: no fluxTest is required.
"""
...
def tecplotMerge(pathRoot,
    ...
    for iItemSeries in range(len(itemSeries)):
        ...
        for iTimeSeriesN in range(iTimeSeriesS, iTimeSeriesE):
            ...
            try:
                for iItemPrefixSeries in range(len(itemPrefixSeries)):
                    fileNow = os.path.join(
                        pathRoot, timeSeries[iTimeSeriesN],
                ...
                print("Failed to load data of item {0} at time {1}.".format(
                    itemNow, timeNow))
                continue

            # Process for imported data
            # Remove or keep duplicate data of same xy but different z
            # Keep 1 or average 2
            if (keepDataOption):
                ...
                if splits.size != 0:
                    dataItemNow = np.stack(list(
                        map(
                            lambda item: np.mean(item, axis=0),
                            filter(
                                lambda item: item.shape[0] == keepDataOption,
                                np.split(dataItemNow, splits, axis=0)))),
                                           axis=0)
            # Sort data after remove duplicate
            ...
            # Use data from 0.02 to 0.025
            yNow = dataItemNow[:, 1]
            UyNow = dataItemNow[:,
                                utils.first(dataItemNameNow,
                                            condition=lambda x: x == 'U_y')]
            iToInterop = np.where((yNow > 0.02) & (yNow < 0.025))[0]
            popt, pcov = scipy.optimize.curve_fit(funCToFit,
                                                  yNow[iToInterop],
                                                  UyNow[iToInterop],
                                                  method='trf')
            ...
            if (writeCombine):
                # write to file
                tecDataPathN = os.path.join(
                    pathRoot, timeSeries[iTimeSeriesN], 'comb-Time_' +
                    timeSeries[iTimeSeriesN] + '-' + itemNow + '.dat')
                with open(tecDataPathN, 'w') as f:
                    file_header = 'TITLE="{0}-{1}"\r\n'.format(
                        timeNow, itemNow)
                    variables = 'VARIABLES="{0}" '.format(
                        '" "'.join(dataItemNameNow
                                   )) + '"U_m (kr + b)" "U_m (Ar + B/r)" \r\n'
                    zone_header = 'ZONE T="{0}-{1}" I={2} J={3}'.format(
                        timeNow, itemNow, dataItemNow.shape[0],
                ...
                with open(tecDataPathN, 'ab') as f:
                    ...
                    np.savetxt(f,
                               ...
                               newline='\n')
                    np.savetxt(f,
                               funCToFit(dataItemNow[:, 1] + 1E-30, *popt),
                               delimiter=',',
                               newline='\n')

            # Process for rf data to output
            if cntTimeSeriesLoad[iItemSeries, 0] == 0:
                ...
                dataItemRFSeries = np.zeros(
                    (cntTimeSeriesLoad.shape[1], iTimeSeriesE - iTimeSeriesS,
                     len(dataItemRFSeriesName)))
            ...
            for iIsoSdSurfaceValue in range(lenIsoSdSurfaceValue):
                ...
                for iIsoItemDataIndex in range(1,
                                               len(dataItemRFSeriesName) - 1):
                    dataItemRFSeries[iIsoSdSurfaceValue,
                                     cntTimeSeriesLoad[iItemSeries,
                                                       iIsoSdSurfaceValue],
                                     iIsoItemDataIndex] = np.interp(
                                         sFlameItemNow, sItemNow,
                                         dataItemNow[:, iIsoItemDataIndex - 1])
                # else:
                ...
                dataItemRFSeries[
                    iIsoSdSurfaceValue, cntTimeSeriesLoad[iItemSeries,
                                                          iIsoSdSurfaceValue],
                    len(dataItemRFSeriesName) - 1] = funCToFit(
                        dataItemRFSeries[iIsoSdSurfaceValue, cntTimeSeriesLoad[
                            iItemSeries, iIsoSdSurfaceValue], 2] + 1E-30,
                        *popt)
            ...
            # Here for Qdot or curl Max for rf position
            QdotNm = np.max(dataItemNow[:, iQdot])
            # iCurl = utils.first(dataItemNameNow,
            #                     condition=lambda x: x ==
            #                     ('cur_sd' + isoSdSurface))
            index_QdotmList = np.where(
                np.logical_and(
                    np.diff(np.diff(dataItemNow[:, iQdot])) < 0,
                    dataItemNow[1:-1, iQdot] > 0.1 * QdotNm))[0]
            if index_QdotmList.size == 0:
                continue
            # iFlameDataItemNow = index_QdotmList[np.argmax(
            #     dataItemNow[index_QdotmList, iQdot])]
            iFlameDataItemNow = np.argmax(dataItemNow[:, iQdot])
            dataItemRFSeries[lenIsoSdSurfaceValue,
                             cntTimeSeriesLoad[iItemSeries,
                                               lenIsoSdSurfaceValue],
                             0] = timeNow
            dataItemRFSeries[lenIsoSdSurfaceValue,
                             cntTimeSeriesLoad[iItemSeries,
                                               lenIsoSdSurfaceValue],
                             1:len(dataItemRFSeriesName) -
            ...
            dataItemRFSeries[
                lenIsoSdSurfaceValue, cntTimeSeriesLoad[iItemSeries,
                                                        lenIsoSdSurfaceValue],
                len(dataItemRFSeriesName) - 1] = funCToFit(
                    dataItemRFSeries[lenIsoSdSurfaceValue, cntTimeSeriesLoad[
                        iItemSeries, lenIsoSdSurfaceValue], 2] + 1E-30, *popt)
        ...
        dataItemRFSeriesName.extend([
            "x1", "y1", "z1", "dx1/dt", "dy1/dt", "dz1/dt", "x2", "y2", "z2",
            "dx2/dt", "dy2/dt", "dz2/dt"
        ])
        # dataRGrid, dataTGrid, dataRTFlux, dataRTFluxName = obtainFlux(
        #     os.path.join(pathRoot, "../fluxTest", timeSeries[-1]))
        # areaInterp = scipy.interpolate.interp2d(dataRGrid,
        #                                         dataTGrid,
        #                                         dataRTFlux[:, 2].reshape(
        #                                             (len(dataTGrid), -1)),
        #                                         kind='cubic')
        # rhoInterp = scipy.interpolate.interp2d(dataRGrid,
        #                                        dataTGrid,
        #                                        dataRTFlux[:, 3].reshape(
        #                                            (len(dataTGrid), -1)),
        #                                        kind='cubic')
        # rhoUInterp = scipy.interpolate.interp2d(dataRGrid,
        #                                         dataTGrid,
        #                                         dataRTFlux[:, 4].reshape(
        #                                             (len(dataTGrid), -1)),
        #                                         kind='cubic')
        # dataItemRFSeriesName.extend(dataRTFluxName[2:])

        otFileItem = os.path.join(
            pathRoot, "{0}_S{1}_E{2}_iso{3}_rf_sgF{4}_avWid{5}.dat".format(
                itemNow, timeSeries[iTimeSeriesS],
                timeSeries[iTimeSeriesE - 1], isoSdSurface, savgolFilterWidth,
                averageWdith))
        with open(otFileItem, "w") as f:
            otFileHeader = 'TITLE="Rf for {0} from {1} to {2}."\r\n'.format(
                itemNow, timeSeries[iTimeSeriesS],
                timeSeries[iTimeSeriesE - 1])
        ...
        for iIsoSdSurfaceValue in range(cntTimeSeriesLoad.shape[1]):
            dataItemIsoRFSeries = dataItemRFSeries[
                iIsoSdSurfaceValue, 0:cntTimeSeriesLoad[iItemSeries,
                                                        iIsoSdSurfaceValue], :]
            ...
            try:
                ...
                index = np.logical_and(
                    np.abs(np.diff(dataItemIsoRFSeriesTXYZ[:, 1])) < 1E-20,
                ...
                dataItemIsoRFSeriesUVW = utils.moving_derivative(
                    dataItemIsoRFSeriesTXYZ[:, 1:4], averageWdith, 2,
            ...
            try:
                ...
                dataItemIsoRFSeriesAppend2[:, 3:6] = utils.moving_derivative(
                    dataItemIsoRFSeries[:, 1:4], averageWdith, 2,
                    dataItemIsoRFSeries[:, 0])
            except:
                pass
            ...
            # dataItemIsoRFSeriesAppendRT = np.zeros(
            #     (dataItemIsoRFSeries.shape[0], 3))
            ...
            # dataItemIsoRFSeries = np.concatenate(
            #     (dataItemIsoRFSeries, dataItemIsoRFSeriesAppendRT), axis=1)
...
if __name__ == "__main__":
    ...
    parse.add_argument('--case',
                       action='store',
                       nargs=1,
                       type=utils.check_path,
                       default='.',
                       help='Root path.')
    parse.add_argument('--timeStart',
                       action='store',
                       nargs=1,
                       type=float,
                       default=[0.0],
                       help='Starting time.')
    parse.add_argument('--timeEnd',
                       action='store',
                       nargs=1,
                       type=float,
                       default=[-1.0],
                       help='Ending time.')
    parse.add_argument('--isoSdSurface',
                       action='store',
                       nargs=1,
                       type=str,
                       help='Iso-surfaces for Sd evaluation.')
    parse.add_argument('--isoSdSurfaceValue',
                       action='store',
                       nargs='*',
                       type=float,
    ...
    parse.add_argument(
        ...
        type=int,
        default=[32],
        help='Determine whether write combined file for each step.')
    ...
    parse.add_argument('--savenpz',
                       action='store',
                       nargs=1,
                       type=utils.str2bool,
                       default=[False],
                       help='Determine whether save combined npz file.')
    ...
    parse.add_argument(
        ...
        type=int,
        default=[0],
        help='Determine kept data point. 0 for z = 0; 1 for z = +-; 2 for all.'
    )

    parse.add_argument(
        '--writeTimeStepCombine',
        action='store',
        nargs=1,
        type=utils.str2bool,
        default=[False],
    ...
    print(
        "Merge timeseries data in {0} from time {1} to {2} for iso {3} = {4}.".
    ...
    tecplotMerge(pathRoot, args.timeStart[0], args.timeEnd[0],
                 args.isoSdSurface[0], args.isoSdSurfaceValue, args.savenpz[0],
                 args.writeTimeStepCombine[0], args.averageWidth[0],
                 args.keepDataOption[0], args.savgolFilterWidth[0])
