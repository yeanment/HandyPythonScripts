...
if __name__ == "__main__":
    ...
    pathRootU2 = os.path.abspath(
        '/mnt/m/OpenFOAM-1712-Diff/H2Ig-phi_05.10-dilution_00.00/' +
        'IgnitionMIE_Upwind/H2Ig_V2.0_L20.0T2_D4.0E-4_T2.0E-4_C0.0E-03_PLOT/' +
        'S9.0E10/postProcessing')
    pathRootU4 = os.path.abspath(
        '/mnt/m/OpenFOAM-1712-Diff/H2Ig-phi_05.10-dilution_00.00/' +
    ...
    pathToSave = os.path.abspath(
        ...
        'H2IgMIEUpwind_L20.0T2_D4.0E-4_T2.0E-4_U0U2U4/' + 'pythonPDF')
    ...
    dataItemNowU2, dataItemNameNowU2 = obtainCombinedData(
        pathRootU2, timeNow, isoSdSurface, isoSdSurfaceValue, keepDataOption)
    dataItemNowU4, dataItemNameNowU4 = obtainCombinedData(
        pathRootU4, timeNow, isoSdSurface, isoSdSurfaceValue, keepDataOption)
    ...
    for i in range(dataItemNowU0.shape[1] - 7, dataItemNowU0.shape[1]):
        varName = dataItemNameNowU0[i]
        ...
        with open(fileNow, 'w') as f:
            ...
            otZoneHeader = 'ZONE T="{2}" I={0} J={1}\r\n'.format(
                500, 1, varName +
                " U0") + 'ZONETYPE = Ordered, DATAPACKING = BLOCK\r\n'
            f.writelines([otZoneHeader])
            density = scipy.stats.gaussian_kde(dataItemNowU0[:, i])
            dist = np.max(dataItemNowU0[:, i]) - np.min(dataItemNowU0[:, i])
            var = np.linspace(
                np.min(dataItemNowU0[:, i]) - dist * 2,
                np.max(dataItemNowU0[:, i]) + dist * 2, 500)
            np.savetxt(f, var, delimiter=',', newline='\n')
            np.savetxt(f, density(var), delimiter=',', newline='\n')

            otZoneHeader = 'ZONE T="{2}" I={0} J={1}\r\n'.format(
                500, 1, varName +
                " U2") + 'ZONETYPE = Ordered, DATAPACKING = BLOCK\r\n'
            f.writelines([otZoneHeader])
            density = scipy.stats.gaussian_kde(dataItemNowU2[:, i])
            dist = np.max(dataItemNowU2[:, i]) - np.min(dataItemNowU2[:, i])
            var = np.linspace(
                np.min(dataItemNowU2[:, i]) - dist * 2,
                np.max(dataItemNowU2[:, i]) + dist * 2, 500)
            np.savetxt(f, var, delimiter=',', newline='\n')
            np.savetxt(f, density(var), delimiter=',', newline='\n')

            otZoneHeader = 'ZONE T="{2}" I={0} J={1}\r\n'.format(
                500, 1, varName +
                " U4") + 'ZONETYPE = Ordered, DATAPACKING = BLOCK\r\n'
            f.writelines([otZoneHeader])
            density = scipy.stats.gaussian_kde(dataItemNowU4[:, i])
            dist = np.max(dataItemNowU4[:, i]) - np.min(dataItemNowU4[:, i])
            var = np.linspace(
                np.min(dataItemNowU4[:, i]) - dist * 2,
                np.max(dataItemNowU4[:, i]) + dist * 2, 500)
            np.savetxt(f, var, delimiter=',', newline='\n')
            np.savetxt(f, density(var), delimiter=',', newline='\n')
        ...
        pdfPlotFileNow = os.path.join(pathToSave, "PDFPLOT_" + itemNow,
                                      timeNow, dataItemNameNowU0[i] + ".svg")
        g = plt.figure()
        ax = sns.kdeplot(dataItemNowU2[:, i],
                         label="$a_g = 400 s^{{-1}}$, mean = {0:.4f}".format(
                             np.mean(dataItemNowU2[:, i])))
        ax = sns.kdeplot(dataItemNowU4[:, i],
                         label="$a_g = 800 s^{{-1}}$, mean = {0:.4f}".format(
                             np.mean(dataItemNowU4[:, i])))
        ybound = ax.get_ylim()
        plt.close(g)
        g = plt.figure()
        ax = sns.kdeplot(
        ...
        ax = sns.kdeplot(
            dataItemNowU2[:, i],
            color='blue',
            label="$\\bf{a_g = 400 s^{{-1}}}$"
            # + ", mean = {0:.4f}".format(np.mean(dataItemNowU2[:, i]))
        )
        ax = sns.kdeplot(
            dataItemNowU4[:, i],
            color='black',
            label="$\\bf{a_g = 800 s^{{-1}}}$"
            # + ", mean = {0:.4f}".format(np.mean(dataItemNowU4[:, i]))
        )
        ax.set_ylim(ybound)
        legend = ax.legend(loc='best')
        ax = sns.rugplot(dataItemNowU0[:, i], height=.5, color='red', linewidth=5)
        # ax = sns.rugplot(dataItemNowU2[:, i], height=.15, color='blue')
        # ax = sns.rugplot(dataItemNowU4[:, i], height=.1, color='black')
        ax.set_xlabel("$\\bf{" + toPlotLabel[i - dataItemNowU0.shape[1] + 7] + "}$")
        ax.set_ylabel("\\bf{PDF}")
        # loc = ax.get_xticks()    #returns the current tics and lables.
        # min_new = loc[0] - (loc[1]-loc[0])
        # max_new = loc[len(loc)-1] + (loc[1]-loc[0])
        # ax.set_xlim(left=min_new, right=max_new)
