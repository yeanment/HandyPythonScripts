...
def tecplotMerge(path_root, time_start, time_end, isoSurface, isoValue, isoZBilgerValue):
    try:
    ...
    iso_file_ext = '_iso' + isoSurface + isoValue + '.raw'
    ...
    for time_index in range(start_index, end_index):
        ...
        # write to file 
        tecdataN_pathN = os.path.join(data_pathN, 'data' + '_iso' + isoSurface + isoValue + '.dat')
        with open(tecdataN_pathN, 'w') as f:
            file_header = 'TITLE="{0}_{1}_{2}"\r\n'.format(time_now, isoSurface, isoValue)
        ...
        index_isoZBilgerN = np.argmin(np.abs(ZBilgerN[:, 3] - isoZBilgerValue))
        dataf[iter_e, 0] = time_now
        dataf[iter_e, 1:] = dataN[index_isoZBilgerN, :]
        iter_e += 1
        
    dataf = np.nan_to_num(dataf)    
    dataf = dataf[0:iter_e, :]
    tecflamedata = os.path.join(path_root, "S{0}-E{1}-iso{2}_{3}-isoZBilger_{4}.dat".format(\
        time_series[start_index], time_series[end_index-1], isoSurface, isoValue, isoZBilgerValue))
    with open(tecflamedata, 'w') as f:
        file_header = 'TITLE="S{0}-E{1}-iso{2}_{3}-isoZBilger_{4}"\r\n'.format(\
            time_series[start_index], time_series[end_index-1], isoSurface, isoValue, isoZBilgerValue)
        variables = 'VARIABLES="TIME" "{0}"\r\n'.format('" "'.join(item_data))
        zone_header = 'ZONE T="iso{0}_{1}-isoZBilger_{2}" I={3} J={4}\r\n  \
            ZONETYPE = Ordered, DATAPACKING = BLOCK\r\n'.format( \
            isoSurface, isoValue, isoZBilgerValue, dataf.shape[0], 1)
        f.writelines([file_header, variables, zone_header])
...
if __name__ == "__main__":
    ...
    if '--ZBilger' in sys.argv:
        isoValue_index = sys.argv.index('--ZBilger') + 1
        try:
            isoZBilgerValue = float(sys.argv[isoValue_index])
            # print(type(isoValue))
        except BaseException as e:
            print('Error in input isoValue.')
            exit()
    ...
    tecplotMerge(path_root, time_start, time_end, isoSurface, isoValue, isoZBilgerValue)
