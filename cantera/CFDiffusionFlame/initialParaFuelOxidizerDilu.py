...
if __name__ == "__main__":
    ...
    # Given Fuel:Oxid:Dilu = 1:stoichOF:qDF
    # Given H2:O2:N2 = 2:1:N = 1:0.5:qDF
    qDF = 9.
    # Given Zst of initial mixture
    Zst = 0.8
    phiS = 1 / Zst - 1
    YFF = (phiS + 1) / (MWD / MWF * qDF + 1 + nuOF)
    YOO = nuOF * YFF / phiS
    YFD = 1 - YFF
    YOD = 1 - YOO
    ...
    data_directory = 'EdgeFlameFuelOxidDiluZ/' + fuel + '-' + oxidizer + '-' + diluent + '/'
    ...
    sim.restore(filename="forrestart.xml", loglevel=loglevel)
    ...
    # sim.set_refine_criteria(ratio=20.0, slope=0.8, curve=0.8, prune=0.8)
    ...
    sim.solve(loglevel, refine_grid=refine_grid)
    print("Data size of run 1:", len(sim.grid))
    sim.write_csv(data_directory +
                  'phiS_{0:.2f}-qDF_{1:.2f}-XH2_{2:.2f}-Premixed.csv'.format(
                      phiS, qDF, XFF),
                  species='Y')
