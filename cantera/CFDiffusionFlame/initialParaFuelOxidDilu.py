...
if __name__ == "__main__":
    ...
    parse.add_argument('--DilutionRatio', action='store', nargs=1, type=float, \
        default = [7.5], help='qDilu, Fuel:Oxid:Dilu = 1:stochi:qDilu.')
    ...
    fuel, oxidizer, diluent = 'H2', 'O2', args.Diluent[0]
    ...
    data_directory = 'data/EdgeFlameFuelOxidDiluZst/{0}-{1}-{2}/'.format(
    ...
    gas.equilibrate('HP')
    rhob = gas.density
    print('// {0} : {1} : {2} = {3:.4f} : {4:.4f} : {5:.4f}'.format(
        fuel, oxidizer, diluent, 1, stoichOF, qDilu))
    ...
    sim.write_csv(data_directory +
                  '{0}-qDilu_{1:.4f}-Zst_{2:.4f}-Premixed-{3}.csv'.format(
                      diluent, qDilu, Zst, args.TransportModel[0]),
                  species='Y')
    ...
    # sim.show_stats
    dataSaveDir = data_directory + '{0}-qDilu_{1:.4f}-Zst_{2:.4f}-Premixed-{3}'.format(
        diluent, qDilu, Zst, args.TransportModel[0])
    ...
    while True:
        ...
        try:
            ...
            print(
                'Flame established at iter = {0} with iter_UO = {1}, T_f = {2}'
